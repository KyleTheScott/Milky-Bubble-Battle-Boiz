...
public class PlaySelection : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Submit"))
        {
            ...
            {
                Debug.LogWarning("Can't select gamemodes that we dont have yet!  Wanna get rid of this message go delete this if/else statement in PlayerSelection.cs");
            }
        ...
        {
            index = 0;
            menuController.ChangeScreen(menuController.PlayOption, menuController.MainMenu);
    ...
    public void ChangeMenus() //switching menus,  similar to how change options is built
    {
        if (index == 0)
        {
            menuController.ChangeScreen(menuController.PlayOption, menuController.MapSelect);
        }
