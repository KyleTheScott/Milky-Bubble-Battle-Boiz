...
public class Player : MonoBehaviour
{
    ...
    private void OnMove(InputValue value)
    {
        if (!isAbsorbing && !isStunned)
        {
            ...
            movementVector = new Vector3(moveVector.x, 0.0f, moveVector.y);
        }
    }

    private void OnRotate(InputValue value)
    {
        if (playerInput.currentControlScheme.Contains("Mouse"))
        {
            ...
            {
                pointToLookVector = cameraRay.GetPoint(rayInfo.distance);
            }
        }
        else
        {
            Vector2 rotationVector = value.Get<Vector2>().normalized;
            pointToLookVector = new Vector3(rotationVector.x, 0, rotationVector.y) * 5 + transform.position;
        }  
    }
    ...
    private void Update()
    {
        ...
        transform.LookAt(new Vector3(pointToLookVector.x, transform.position.y, pointToLookVector.z));
    }
    ...
    //                isAbsorbing = true;
    //                // TODO: Figure out a method to bring the sphere to the player
    //                targetPlatform.StartCoroutine(targetPlatform.BecomeAmmo(transform.position));
